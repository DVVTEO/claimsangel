...
class NewProspectProfile
{
    ...
    public function add_admin_menu() {
        // Set a slug for the parent menu
        $parent_slug = 'prospects';

        // Create the parent menu item "Prospects"
        add_menu_page(
            __('Prospects', 'claims-angel'),  // Page title
            __('Prospects', 'claims-angel'),  // Menu title
            'manage_options',                 // Capability
            $parent_slug,                     // Menu slug
            [$this, 'render_admin_page'],     // Callback (make parent clickable)
            'dashicons-businessman',          // Icon
            3                                 // Position (adjust as needed)
        );
        
        // Get the prospect count more efficiently using get_prospect_count()
        ...
        if ($user_country_code) {
            $prospect_count = $this->get_prospect_count($user_country_code);
        }

        $submenu_title = sprintf(__("New Prospects (%d)", "claims-angel"), $prospect_count);

        // Add the submenu item "New Prospects" under the "Prospects" parent.
        add_submenu_page(
            $parent_slug,                     // Parent slug
            __('New Prospects', 'claims-angel'), // Page title
            $submenu_title,                   // Menu title with count
            'manage_options',                 // Capability
            'business-crm',                   // Submenu slug (used in your render_admin_page callback)
            [$this, 'render_admin_page'],     // Callback function
        );

        remove_submenu_page($parent_slug, $parent_slug);

        // Also update the registered admin page details.
    ...
    public function enqueue_admin_assets($hook)
    {
        ...
        if ($hook !== "prospects_page_business-crm") {
            return;
        }
